...
Methodology for finding the b-023 bug class — a request/message handler
...
## What this method found in b-023 (reference — re-verify against the live
code before citing, as line numbers may drift)

`repair_oos_accounts` and `binary_repair_oos_accounts`
(`repairMissingAccountsBinary`) in
`src/state-manager/AccountPatcher.ts` (shardeum `core`, formerly
`shardus-core`). Both loop `for (i = 0; i < receivedBestVote.account_id.length; i++)`
over `receivedBestVote` = `repairInstruction.receipt2.appliedVote`, a field
supplied directly by the message sender.

- Failure mode 1 (type erasure): `account_id` is typed `string[]` in
  `AppliedVote.ts`, but nothing on either handler's path runs
  `Array.isArray(receivedBestVote.account_id)` before the loop — a plain
  object `{ length: 1e46 }` passes through unchanged.
- Failure mode 2 (dead validation): an ajv schema
  (`src/types/ajv/RepairMissingAccountsReq.ts`) already declares
  `account_id: { type: 'array', items: { type: 'string' } }`, but every
  `verifyPayload(...)` call site for this request type is commented out
  in both `AccountPatcher.ts` and `RepairOOSAccountsReq.ts` — and even if
  re-enabled, the call sites pass the wrong registry key
  (`'RepairOOSAccountsReq'`) against the name the schema is actually
  registered under (`'RepairMissingAccountsReq'`).
- Failure mode 3 (binary format bypassed for one field): the "binary"
  handler's request type (`RepairOOSAccountsReq`) uses real field-by-field
  binary writes for most fields, but serializes the `receipt2` field
  (containing `appliedVote`/`account_id`) via
  `Utils.safeStringify(obj)` / `Utils.safeJsonParse(stringified)`
  (`AppliedReceipt2.ts`) instead of the properly bounded, length-prefixed
  binary codec that already exists for `AppliedVote`
  (`AppliedVote.ts`, which caps `account_id.length` at `UInt16`/65535)
  — so the "binary" endpoint has exactly the same exposure as the plain
  JSON one for this field, despite looking more strongly typed.
- Failure mode 4 (float precision, not just "missing bound"): with
  `account_id: { length: 1e46 }`, once `i` exceeds
  `Number.MAX_SAFE_INTEGER` (2^53-1 ≈ 9.007e15), `i++` is a no-op in
  IEEE-754 double arithmetic — the loop is mathematically infinite, not
  merely very long.
- Reachability: reaching the loop requires a valid signature from a node
  that's in the target transaction's `executionGroupMap`
  (`this.crypto.verify(...)`), i.e. the attacker needs to control one
  active, staked validator node — a real but permissionless-network-level
  bar, matching the original report's PoC (which signs with a real node's
  `secrets.json`).
- Blast radius: the handler is present on every validator node; the same
  message replayed against each one is a total, simultaneous network
  shutdown, since Node.js is single-threaded and the hung event loop never
  throws or times out (both handlers' `try/catch` never fires).

Full write-up: `bugs/b-023/answers/analysis.md`.
